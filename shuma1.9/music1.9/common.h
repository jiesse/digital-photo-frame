...
extern int display_part(char *filename,int x, int y, fb_info fb_inf);
extern int display_screen(char *filename, fb_info old_inf, int k);
extern int display_kaiji(fb_info fb_inf);
extern int display_music_jpeg(fb_info fb_inf,int x1,int y1,int x2,int y2);
...
extern int clean_string (const char *buf, int x, int y, fb_info fb_inf,u32_t color);
