...
int display_font(const char *code,int *len,int x,int y,fb_info fb, u32_t color)
{
    ...
    //for (row = 0; row < face->glyph->bitmap.rows; ++row,y++,x--)	{
...
//		;            
		for (pixel = 0; pixel < face->glyph->bitmap.width; ++pixel)		{
            ...
            //else
            //fb_pixel(fb, x+pixel, y, buf32[x+pixel + y * fb.w]);
		}
	}
	return 0;
}
int clean_font(const char *code,int *len,int x,int y,fb_info fb, u32_t color)
{
	unsigned int ucode;
    //FT_UInt glyph_index;
	int row, pixel;
	...
	u8_t *buf24 = decode_jpeg("music8.jpg", &jpeg_inf);
	u8_t * scale_buf = scale24(buf24, fb, jpeg_inf);
    buf32 = rgb24to32(scale_buf, fb);
...
int clean_string (const char * buf, int x, int y, fb_info fb_inf,u32_t color)
{
int len;
	while (buf[0] != '\0') 	{	
		if (clean_font(buf,&len,x,y,fb_inf, color)) return 1;
		buf += len;
		x += psize;
        //y += psize;
	}
